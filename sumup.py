...
def plot(data, wer=False):
    if wer:
        ...
        plot_param(data, title="WER model streaming", ylabel="WER", key='wer_score', output_path='plots/wer/', hardware="koios", device="gpu", vad="NoVAD",method="greedy", condition_on_previous_text="NoCondition", data_type="speech", model_size=None, offline="streaming", compute_type="best")
        ...
        plot_param(data_gpu, title="Latency depending on model on 1080TI (GPU) for faster-whisper", key='segment_latency',output_path='plots/gpu/koios/faster', hardware="koios", device="gpu", compute_type="int8",backend="faster", method="greedy", vad="NoVAD", condition_on_previous_text="NoCondition", data_type="speech", model_size=None)
